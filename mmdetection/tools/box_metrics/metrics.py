...
def get_fast_aji(true, pred):
    ...
    # 合到一张图上
    # 在一张图上进行编码 然后保存numpy
    # labels 的 segment area也是一个一个的 处理后保存为numpy
    # 获取到inst map 之后拿到指标

    for true_id in true_id_list[1:]:  # 0-th is background
        t_mask = true_masks[true_id]
        pred_true_overlap = pred[t_mask > 0] # 选出该mask对应的区域
        pred_true_overlap_id = np.unique(pred_true_overlap) # 计算overlap
...
#####
def remap_label(preds, by_size=False):
    ...
    """
    # numpy中的每一个维度作为一个mask 
    pred = np.zeros(preds.shape[-2:])
    count = 0
    for p in preds:
        count += 1
        pred[p > 0] = count 
