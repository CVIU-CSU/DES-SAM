...
class SamDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self,
        ...
        # label
        self.mask_labels = torch.load(mask_labels)
        self.crop_size = crop_size

    def centerCrop(self,masks):
        center_h = masks.shape[1]/2
        center_w = masks.shape[2]/2
        offset_h = int(center_h - self.crop_size[0]/2)
        offset_w = int(center_w - self.crop_size[1]/2)
        crop_y1, crop_y2 = offset_h, offset_h + self.crop_size[0]
        crop_x1, crop_x2 = offset_w, offset_w + self.crop_size[1]
        return masks[:,crop_y1:crop_y2, crop_x1:crop_x2]
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if training:
            ...
            for im_i, data_sample in enumerate(data_samples):
                per_im_bboxes = data_sample.gt_instances.bboxes.cpu()
                h,w = data_sample.ori_shape
                scale_factor_h,scale_factor_w = data_sample.scale_factor
                if per_im_bboxes.shape[0] > 0:
                    per_im_masks = self.mask_labels[data_sample.img_id]
                    # centerCrop
                    per_im_masks = self.centerCrop(per_im_masks.cpu().numpy().astype(np.uint8))
                    per_im_masks = BitmapMasks(per_im_masks, h,w)
                    rescale_im_masks = per_im_masks.rescale((h*scale_factor_h,w*scale_factor_w))
                    pad_im_masks = rescale_im_masks.pad(data_sample.pad_shape, pad_val=0)
                    data_sample.gt_instances.masks = pad_im_masks
                else:
        ...
        if training:
            ...
            for im_i, data_sample in enumerate(data_samples):
                ...
                if per_im_bboxes.shape[0] > 0:
                    ...
                    per_im_masks = torch.stack(per_im_masks, dim=0) # [boxnum ,b_img_h,b_img_w]
                ...
                data_sample.gt_instances.masks = BitmapMasks(
                    per_im_masks.cpu().numpy(), b_img_h, b_img_w) # 对应box区域为1.0
                data_sample.gt_instances.pairwise_masks = pairwise_masks # 计算一个全局相似度
        return {'inputs': inputs, 'data_samples': data_samples}

# targets {tgt_masks, tgt_imgs_sim, tgt_imgs, gt_boxes}
@MODELS.register_module()
class SAMInstDataPreprocessor(DetDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if training:
            ...
            # Get origin rgb image for color similarity
            ori_imgs = inputs * self.std + self.mean
            downsampled_imgs = ori_imgs.float()
            # Compute color similarity for pseudo mask generation
            for im_i, data_sample in enumerate(data_samples):
                # TODO: Support rgb2lab in mmengine?
                images_lab = skimage.color.rgb2lab(
                ...
                batch_inds = torch.zeros(len(per_im_bboxes)).type_as(per_im_bboxes)[:, None]
                rois = torch.cat([batch_inds, per_im_bboxes], dim=1)
                clipped_per_img_mask = ROIAlign(img_masks[im_i][None, None],rois, clipped_size,1.0, 0)
                # prepare image similarity
                clipped_per_img = ROIAlign(images_lab, rois, clipped_size, 1.0, 0) 
                gt_img_sim = self.get_images_color_similarity(clipped_per_img, clipped_per_img_mask) 
                # prepare image norm for level set 
                clipped_per_img_norm = (
                    ROIAlign(downsampled_imgs[im_i].unsqueeze(0), rois, clipped_size, 1.0, 0).squeeze(1)
                )
                clipped_per_img_norm = clipped_per_img_norm * clipped_per_img_mask
                # prepare bitmasks
                gt_boxmasks = torch.ones(clipped_size, device=clipped_per_img_norm.device, dtype=torch.float).unsqueeze(0).expand(per_im_bboxes.shape[0], -1, -1)
                ##
                gt_boxmasks = gt_boxmasks.unsqueeze(1).float()
                data_sample.gt_instances.gt_imgs_sim = gt_img_sim
                data_sample.gt_instances.gt_imgs = clipped_per_img_norm
                data_sample.gt_instances.gt_boxmasks = gt_boxmasks
                ####
        return {'inputs': inputs, 'data_samples': data_samples}
