...
##  ADD NEW SAM - ImageEncoder
@TRANSFORMS.register_module()
class ResizeLongestSide(BaseTransform):
    def __init__(self,
    ...
    def _get_output_shape(
            self, img: np.ndarray,
            long_edge_length: Union[int, Tuple[int, int]]) -> Tuple[int, int]:
...
class CenterCrop(BaseTransform):
    ...
    def _filter_boxes(self, patch, boxes):
        ...
        bbox = boxes.tensor
        bbox[np.where(bbox[:,0] < patch[0])[0],0] = patch[0]
        bbox[np.where(bbox[:,1] < patch[1])[0],1] = patch[1]
        bbox[np.where(bbox[:,2] > patch[2])[0],2] = patch[2]
        bbox[np.where(bbox[:,3] > patch[3])[0],3] = patch[3]
    ...
    def _crop_image_and_paste(self, image, center, size):
        ...
        patch = np.array((int(x0), int(y0), int(x1), int(y1))) # 边界
        # 中心点距离上下左右的距离
        ...
        while True:
            ...
            for i in range(50):
                ...
                if len(gt_bboxes) == 0:
                    ...
                    results['ori_shape'] = cropped_img.shape[:2]
                    return results
